...
TABLE_EXTRACTION_PROMPT = """Please extract the tables from the images and provide the data in XML format. Some images may not contain tables, and may contain a mix of figures, graphs and equations. Please ignore these images and give them a score of 0. You will structure your response as a JSON object with the following schema:

'table_text': The XML formatted text extracted from the table, use visual cues to separate the columns and rows. Some cells may span multiple columns. Ensure that symbols and greek characters are preserved, do not swap "α" to "a" for example.
